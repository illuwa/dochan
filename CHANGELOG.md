...
정답지로 삼는 `scripts/compare_hwp_pairs.py`)을 실물 78쌍에 돌려 찾은 결함 세 가지를 고쳤다.
...
- **셀 안 중첩 표 소실**: 중첩 표를 문단으로 펴버려 HWP 는 중첩 표를 한 개도 내지 못했다
  ...
  0.980 → 0.999. 중첩 표가 깊이·셀 예산을 넘겨 실패하면 그 셀 내용만 비우고 바깥 표와 다른 셀은
  살린다(종전에는 바깥 표까지 통째로 비웠다; PDF 중첩 표와 같은 정책, 예산 되돌림은 그대로)
