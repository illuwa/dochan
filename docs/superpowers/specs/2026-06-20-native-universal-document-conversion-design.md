...
- Phase 5-A completed (2026-09-07): 한글 줄바꿈 결합 공백을 문자 통계 모델로 판정 — 공개 코퍼스 학습 모델(209KB, 패키지 데이터), 결합 정확도 0.607 → 0.930(79쌍 표본 외), 토큰 유사도 0.961. 남은 Phase 5 과제(중첩 표 트리, 페이지 걸침 병합, 세로쓰기, 괘선 없는 표)는 docs/superpowers/plans/2026-09-07-pdf-phase5-nesting-pagination-vertical-spacing.md 참조.
- Not yet completed for PDF LZW filter, TIFF predictor, 진짜 사용자 암호가 필요한 문서(빈/소유자 경로로 못 여는 경우), 괘선 없는 표(선 없음 표), 중첩 표의 독립 트리 복원(현재 바깥 격자로 평탄화), 페이지에 걸친 표 병합, 세로쓰기 읽기 순서, 병합 셀 완전 일치(현재 행·열이 맞는 병합 표 기준 부분 일치 — 벤치마크 문서 참조), 한 글자 문맥으로 결정되지 않는 줄바꿈 공백(약 7%).
