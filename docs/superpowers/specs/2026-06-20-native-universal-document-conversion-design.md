...
- Not yet completed for PDF LZW filter, TIFF predictor, 진짜 사용자 암호가 필요한 문서(빈/소유자 경로로 못 여는 경우), 괘선 없는 1×1 글상자, 부모와 테두리를 공유하는 연결형 중첩 표(하위 격자 추정은 오탐 211건으로 폐기), WMode 1 세로 폰트, 페이지 경계에서 잘린 셀의 결합, 병합 셀 완전 일치(현재 행·열이 맞는 병합 표 기준 부분 일치), 한 글자 문맥으로 결정되지 않는 줄바꿈 공백(약 6%).
