...
- Phase 4 completed (2026-09-07, 1.4.0): 그래픽 상태(CTM, q/Q, cm) 추적으로 텍스트 조각을 장치 좌표에 배치(한글 'PDF로 저장' 출력의 `q … cm BT … ET Q` 런 구조 대응), 글리프 폭 레이아웃·bold/italic·이미지 XObject 추출+OCR(1.3.0), 경로 연산자(m/l/re + S)에서 축 정렬 괘선 수집 → 군집화 → 연결 성분 → 격자 → 병합 셀(가로/세로, L자형 행별 분할) → 셀 텍스트 배치, 바깥 괘선 없는 표의 괘선 범위 확장, 꽉 찬 줄 기반 문단 병합(항목 표식 분리, 한글·숫자 무공백 결합), 본문 기준 제목 판정, MacRomanEncoding 단순 폰트. 자원 상한: 페이지 콘텐츠 합계 64MB, 선분 2만, 교차 검사 200만, 표 셀 페이지 5만·문서 20만, 성분 선 2천. 검증(scripts/compare_pdf_pairs.py, docs/benchmarks/2026-09-07-pdf-layout-tables-real-doc-validation.md): 동일 문서 HWPX↔PDF 79쌍 공백 토큰 유사도 0.032 → 0.936(최저 0.802), PDF 표 0 → 868개(HWPX 845개), 셀 텍스트 일치율 0.894, 표 구조 완전 일치 0.619. codex 독립 리뷰(P1 3건·P2 4건)와 Opus 감수 반영.
...
- Not yet completed for PDF LZW filter, TIFF predictor, 진짜 사용자 암호가 필요한 문서(빈/소유자 경로로 못 여는 경우), 괘선 없는 표(선 없음 표), 중첩 표의 독립 트리 복원(현재 바깥 격자로 평탄화), 페이지에 걸친 표 병합, 세로쓰기 읽기 순서, 병합 셀 완전 일치(현재 행·열이 맞는 병합 표 기준 부분 일치 — 벤치마크 문서 참조).
