...
PDF 리더가 페이지마다 반복되는 머리글과 바닥글(쪽번호)을 알아보고 본문에서 분리한다. README
비교표의 PDF `머리글/바닥글` 항목이 실물 검증을 거쳐 ✅ 가 됐다.

### 추가 (PDF — 반복 머리글/바닥글 검출)

- **반복 머리글·바닥글 검출**: 페이지 상·하단 12% 구역의 연속 가장자리 블록(최대 3줄)만 후보로
  삼고, 키 (구역, 정규화 텍스트, 글자 크기) 가 `max(2, ⌈0.3 × 텍스트 페이지⌉)` 페이지 이상 반복되면
  반복 머리글/바닥글로 본다. 숫자는 쪽번호형 줄(`3`, `- 3 -`, `3 / 20`, `Page 3`, `3쪽`)에서만 접고
  날짜·조문 번호·`[별지 제1호 서식]` 같은 본문 줄은 숫자를 유지해 구분한다. 쪽번호는 페이지 순으로
  비감소인지 확인한다. 1쪽 문서·회전 페이지는 판정하지 않는다. 여백이 좁아 본문 첫 줄이 구역에
  걸리는 문서를 위해, 가장자리 블록이 본문과 2×글자 크기 이상 떨어진 쪽이 출현의 과반이어야 하고
  (독립 리뷰·감수 반영), 넓은 간격의 세그먼트가 3개 이상인 표형 줄은 후보에서 뺀다. 글자 크기는
  그룹 중앙값 ±1 로 묶어 반올림 경계 누락을 막고, 검출 실패는 경고로 강등한다(초안 보관 상한
  5,000 텍스트 페이지·200,000줄)
- **출력 계약**: 첫 출현 페이지 섹션의 맨 앞에 `HeaderFooter(type='header'|'footer')` 를 한 번 넣고
  (HWPX 와 같은 위치; Markdown 은 `<!-- header: … -->` 한 번) 모든 출현 줄을 본문 흐름에서 뺀다.
  리더는 페이지 초안을 모아 문서 단위로 판정한 뒤 문단을 만드는 2단계 흐름으로 바뀌었고, 표 병합·
  텍스트 표 옵션·링크·이미지 순서는 그대로다. 텍스트 페이지가 5,000을 넘으면 검출을 생략한다
- 측정 지표 `hf_hit`·`mean_hf_hit`·`docs_with_hf` 와 오검출 `pdf_hf_extra`·`hf_false` (`scripts/compare_pdf_pairs.py`)
- 검증(79쌍): 머리글 77개(모두 문서 제목)·쪽번호 바닥글 79개 검출, 본문 오제거 0(`hf_false` 0), `hf_hit` 0.9913,
  토큰 유사도 0.9617 → 0.9753(최솟값 0.8278 → 0.8410), 결합 정확도·표 지표는 동일.
  검증 기록: `docs/benchmarks/2026-09-24-pdf-running-headers.md`
