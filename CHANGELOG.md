...
HWP(바이너리) 파서의 정확성 패치. 같은 문서의 HWP↔HWPX 짝을 비교하는 새 검증(HWPX 파서 결과를
정답지로 삼는 `scripts/compare_hwp_pairs.py`)을 실물 76쌍(+내부 실물 문서 2쌍)에 돌려 찾은 결함 세 가지를 고쳤다.
HWPX·PDF·Office 리더의 출력은 바뀌지 않는다.

### 수정 (HWP)

- **문단 텍스트 중복·서식 오배정**: `PARA_CHAR_SHAPE`(글자모양 구간)는 (UINT32 위치, UINT32
  글자모양 ID) 8바이트 쌍인데 6바이트 간격으로 읽어 위치가 뒤섞였고, 겹친 슬라이스가 문단 뒷부분을
  두세 번 중복시키며 bold/italic 을 무작위로 붙였다. 8바이트로 읽고, 위치가 제어문자를 폭대로 센
  원시 WCHAR 단위(확장 컨트롤 = 8)라는 점을 반영해 `parse_para_text` 가 원시→텍스트 오프셋 맵을
  돌려주며, 런 분할은 단조 증가·범위 클램프를 강제해 어떤 입력에도 런 텍스트의 연결이 원문과 같다.
  검증(HWP↔HWPX 76쌍): 텍스트 유사도 평균 0.9013 → 0.9997, 최솟값 0.642 → 0.9927 (개선 75, 회귀 0),
  서식 런 일치율 0.85 → 1.00
- **셀 안 중첩 표 소실**: 중첩 표를 문단으로 펴버려 HWP 는 중첩 표를 한 개도 내지 못했다
  (76쌍: HWPX 51개 vs HWP 0개). HWPX·PDF 와 같이 `Table` 객체를 셀 `paragraphs` 에 읽기 순서대로
  넣는다. 표 수 744 → 795 (정답 796), 표 서명 일치 0.935 → 0.999, 중첩 일치 0 → 1.0, 셀 일치율
  0.980 → 0.999. 중첩 표가 깊이·셀 예산을 넘겨 실패하면 바깥 표와 다른 셀은
  살린다. 비워지는 단위는 그 표를 품은 문단 그룹(같은 문단의 텍스트·형제 표 포함)이다
  (종전에는 바깥 표까지 통째로 비웠다; PDF 중첩 표와 같은 정책, 예산 되돌림은 그대로)
- **이미지 연결 중단**: 압축 플래그가 켜진 문서에도 한컴은 PNG·JPEG 를 압축하지 않고 저장한다
  (실물 85개 문서의 146 스트림 중 52개). 압축 해제 실패를 비압축 데이터로 다루려던 분기가
  `ValueError` 만 잡아 `zlib.error` 가 새면서 그 문서의 이미지 연결이 통째로 중단됐다
  (`WARN: BinData 연결 실패`, 28개 문서). 같은 분기로 잡고 항목별로 격리한다. 경고 문서 28 → 0,
  데이터가 있는 이미지 68 → 113

### 추가

- `scripts/compare_hwp_pairs.py`: 디렉터리 안의 같은 stem `.hwpx`(정답)·`.hwp` 짝을 비교해
  `tok_ratio`·표 서명·`cell_hit`·중첩·서식 런 일치(`format_match`)를 낸다.
  검증 기록: `docs/benchmarks/2026-09-24-hwp-hwpx-parity.md`

## [1.5.0] - 2026-09-24

PDF 리더의 남은 구조 과제를 단계적으로 닫은 릴리스. 줄바꿈으로 갈라진 한글 어절의 공백을
공개 코퍼스 학습 문자 통계로 복원하고(결합 정확도 0.61 → 0.94), 셀 안의 중첩 표를 트리로
복원하며, 페이지에 걸친 표를 하나로 잇고, 세로쓰기·회전 텍스트의 읽기 순서를 다룬다. 괘선
없는 표는 실험적 옵션으로 제공한다. 모든 항목은 알고리즘의 전제조건을 실문서 79쌍에서 먼저
실측해 도달 가능한 상한을 정한 뒤 구현했고, 단계마다 codex 독립 리뷰와 Opus 감수를 거쳤다.
...
- 공백 모델을 완료된 공개 코퍼스 전체(6,779건, 음절쌍 2,439만)로 재학습 — 결합 정확도
  0.932 → 0.938, 모델 524KB

### 추가 (PDF — 중첩 표 트리, 페이지 걸침 병합, 세로쓰기, 괘선 없는 표 옵션)

- **중첩 표의 독립 트리 복원**: 괘선 연결 성분을 포함 관계로 트리화해 채택 표의 단일 셀 안에
  완전히 들어가는 성분을 그 셀의 중첩 `Table` 로 넣는다(JSON 은 트리, Markdown 은 중첩 구분자).
  텍스트는 안쪽 표부터 한 번만 소비하고 셀 안에서 문단과 중첩 표를 y 순서로 배치한다. 빈 서식
  격자(3×3 선·6셀 이상)도 중첩으로 채택. 페이지당 후보 1,000·깊이 32·셀 예산 적용
- **페이지에 걸친 표 병합**: 앞 페이지 마지막 표 아래와 다음 페이지 첫 표 위에 본문이 없고
  (머리말·꼬리말 60pt 제외, 본문 띠 교차 필수), 열 경계가 1.5pt 안에서 같으며, 표 아래 남은
  공간이 다음 행 높이(또는 페이지 높이 12%) 이하일 때만 행을 이어 붙인다. 반복 제목 행(다중 행
  포함) 제거, 셀 provenance 는 원래 페이지 유지, 회전 페이지 제외
...
- 검증(79쌍, 옵션 꺼짐): nested_match 0 → 0.364(24/66 — 괘선으로 도달 가능한 24개 전부),
  괘선 없는 표 옵션은 켜도 다른 지표 변화 없이 표 +8,
...
- 괘선 없는 1×1 글상자, 부모와 테두리를 공유하는 연결형 중첩 표, WMode 1(Identity-V) 세로
  폰트, 페이지 경계에서 잘린 셀의 결합, 5만 셀 한도 근처의 예산 선점, 열 병합만 쓰는 2단
  반복 제목의 둘째 행(데이터 행과 구조로 구별할 수 없어 제거하지 않음)
