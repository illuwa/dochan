...
class Dochan:
    ...
    def _parse_dispatch(self):
        ...
        # Container signatures are authoritative.  Extensions are only a
        # fallback for malformed inputs whose format-specific reader can
    ...
    def _parse_hwp_family(self, ext: str):
        """확장자와 실제 HWP/HWPX 매직이 다르면 실제 내용에 맞춰 라우팅."""
        try:
            with open(self.file_path, 'rb') as f:
                magic = f.read(8)
        except OSError:
    ...
    def _detect_ole_formats(self):
        """Identify an extensionless OLE document from unique standard streams."""
